...
public class test extends Fragment {
    ...
    public void loadPictures(boolean reload) {
        ...
        if (!savedPics.equals("null") && reload && pictureList.size() == PIC_START) {
            pictureList.clear();
    ...
    private class Pictures extends AsyncTask<String, String, JSONObject> {
        ...
        private int old_displayed;
        public Pictures(boolean reload) {
            this.reload = reload;
            this.old_displayed = displayed_pics;
            if (reload)
                displayed_pics = PIC_START;

            //Util.alert(displayed_pics + " - loading", mainActivity);
        }
        ...
        protected JSONObject doInBackground(String... params) {
            ...
            int start = 0;
            int count = PIC_START;
            if(pictureList.size() > PIC_START) {
                start = (pictureList.size() - PIC_COUNT);
                count = PIC_COUNT;
            }
            if(!reload) start += PIC_COUNT;
        ...
        protected void onPostExecute(JSONObject result) {
            ...
            try {
                ...
                displayed_pics = pictureList.size();
    ...
    private void updateListView(JSONObject input, boolean reload) {
        ...
        toggleLoading(false);
        Util.alert(displayed_pics + " - displayed", mainActivity);
        Util.alert(pictureList.size() + " - size", mainActivity);
    }
    ...
    private void loadSavedPics(String result) {
        ...
        if (jArray != null) {
            ...
            //Util.alert(displayed_pics + "before", mainActivity);
            displayed_pics = pictureList.size();
            //Util.alert(pictureList.size() + "size", mainActivity);
            Util.alert(displayed_pics + "after", mainActivity);
        }
    }
    ...
    public class EndlessScrollListener implements OnScrollListener {
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            if (lastItem + 2 == totalItemCount) {
                if (/*preLast != lastItem*/true) { //to avoid multiple calls for last item
                    displayed_pics += PIC_COUNT;
